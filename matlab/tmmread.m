function  [A,idx] = tmmread(filename, reorder, dim)
%
...
if ( strcmp(rep,'coordinate')) %  read matrix given in sparse 
  ...
  if  ( strcmp(field,'real') )               % real valued entries:
    ...
    if nargin > 1 && ~reorder
        if nargin > 2
            A = sparse(T(:,1), T(:,2), T(:,3) , dim , dim);
            idx = unique(T(:,1));
        else
            A = sparse(T(:,1), T(:,2), T(:,3) , rows , cols);
        end
    else
        map = sparse(max(T(:,1)), 1);
        last = 0;
        j = 0;
        dof = 4;
        for i = 1:entries
            if (T(i,1) ~= last)
                if T(i,1) - mod(T(i,1)-1, dof) ~= last - mod(last-1, dof)
                    for k = 1:dof
                        j = j + 1;
                        map(T(i,1) - 1 - mod(T(i,1)-1, dof) + k) = j;
                    end
                end
                last = T(i,1);
            end
        ...
        A = sparse(map(T(:,1)), map(T(:,2)), T(:,3), dim , dim);
    end
