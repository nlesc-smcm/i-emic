...
#ifdef HAVE_ZOLTAN
//!                                                                         
//! this file contains a modified version of the 'rectangularMesh' class    
//! from the Zoltan examples, which has been extended to include vertex-    
//! weights.                                                                
//!                                                                         
...
public:
  ...
  void reset_totals(Teuchos::RCP<Epetra_Map> new_map=Teuchos::null) 
    { 
    map = new_map;
    myIDs.erase(myIDs.begin(), myIDs.end());
    }
  ...
  int get_num_my_IDs()
    ...
    if (myIDs.size() == 0)  
      ...
      if (map==Teuchos::null)
        ...
        {
        if (map->NumGlobalElements()!=numIDs) Error("bad map for mesh!",__FILE__,__LINE__);
        for (int i=0; i<numIDs; i++)
          {
          if (map->MyGID(i))
            {
  ...
  // Constructors
  rectangularMesh(RCP<Epetra_Map> inimap=Teuchos::null)
    {
    this->x_dim   =5;
    this->y_dim   =5;
    this->x_stride=1;
    ...
    this->set_par_env(); 
    this->reset_totals(inimap);
    weights.resize(x_dim*y_dim);
    for (int i=0;i<x_dim*y_dim;i++) weights[i]=get_num_edges(i+1);
    }
  rectangularMesh(int xdim, int ydim, 
             int xstride, int ystride,
             int x1, int y1,std::vector<double>& wgts)
    {
    this->x_dim=xdim;
    this->y_dim=ydim;
    ...
    this->reset_totals();
    weights.resize(x_dim*y_dim);
    for (int i=0;i<x_dim*y_dim;i++) weights[i]=wgts[i];
    }
  ...
  void set_x_dim(int xdim){this->x_dim = xdim; this->reset_totals(map);}
  void set_y_dim(int ydim){this->y_dim = ydim; this->reset_totals(map);}
  ...
  void set_weights(std::vector<double>& wgts)
    {
    weights.resize(x_dim*y_dim);
    for (int i=0;i<x_dim*y_dim;i++) weights[i]=wgts[i];
    }
    ...
    *ierr = ZOLTAN_OK;
    ...
    for (int i=0; i<mesh->get_num_my_IDs(); i++)
      ...
      if (wgt_dim == 1)
        ...
        obj_wgts[i] = mesh->get_weight(globalID[i]);
        }
      }
    return;
    }
  ...
  static void get_geometry_list(void *data, int sizeGID, int sizeLID,
    ...
    *ierr = ZOLTAN_OK;
    ...
    for (int i=0; i<num_obj; i++)
      ...
      geom_vec[2*i] = (double)y;
      geom_vec[2*i + 1] = (double)x;
      }
    return;
    }
  ...
  void draw_partitions(std::ostream& os, char *title, int len, int *gids, int want_wgts)
    {
    int root = 0;
...
/*  if ((this->x_dim > 75) || (this->y_dim > 75))
      {
      if (iam_root) os << "mesh: too big to draw" << endl;
      return;
      }
*/
  ...
  for (int i=0; i < this->nprocs; i++) {
    for (int j=0; j<incounts[i]; j++){
      ...
      if (want_wgts){
        ...
        wgt_total[i] += this->get_weight(ingids[nextId]);
      }
      nextId++;
    }
  }
...
os << "\n\n" << title << "\n\n";
  ...
  for (row=this->y_dim-1; row >= 0; row--){
    for (col=0; col < this->x_dim; col++){
      ...
      os<<parts[local_id]<<" ";
      if (this->x_stride > this->y_stride) os<<" ";
    }
    os<<"\n";
    if (this->y_stride > this->x_stride) os<<"\n";
  }

  if (want_wgts){
    for (int i=0; i<this->nprocs; i++){
      os<<"Total weight partition " << i<<": "<<wgt_total[i]<<std::endl;
    }
    delete [] wgt_total;
  }

  this->get_coords(1, col, row);
  os<<"Geometry: ("<<col<<"x"<<row<<") X ";
...
#else
// no Zoltan, dummy mesh class
class rectangularMesh
  {
  public:
  rectangularMesh(){};
  int get_num_my_IDs(){return 0;}
  int get_row_col(int id,int& row, int& col){return 0;}
  int get_my_ID(int i){return 0;}
  };
