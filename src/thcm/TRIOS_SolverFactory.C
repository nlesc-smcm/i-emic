...
// these classes always exist but are empty if HAVE_PARASAILS/HAVE_MRILU
// are not defined:
#include "TRIOS_ParaSailsPrecond.H"
...
// for storing the level matrices of ML:
  ...
  if (is_ifpack)
    ...
    if (smoo=="IFPACK")
      ...
      if (memcmp(str1,str2,len)==0)
        {
        HYMLS::Tools::Warning("line relaxation no longer supported",__FILE__,__LINE__);
        }
      }
    ...
    // I had something here in trilinos_thcm, but I don't know if it ever worked.
      if (visualize||repartition)
        {
        HYMLS::Tools::Warning("visualization and repartitioning not supported",__FILE__,__LINE__);
        }
...
#else
    HYMLS::Tools::Error("ParaSails is not available, choose another preconditioner!",__FILE__,__LINE__);
#endif
    }
    ...
    {
    HYMLS::Tools::Error("Bad Preconditioner Type: "+PrecType,__FILE__,__LINE__);
    }
...
//#endif //]
    ...
    {
    HYMLS::Tools::Error("Bad preconditiner type: "+PrecType,__FILE__,__LINE__);
    }
  bool eigen_analysis = plist.get("Analyze Preconditioned Spectrum",false);      
...
Teuchos::RCP<AztecOO> SolverFactory::CreateKrylovSolver(Teuchos::ParameterList& plist,int verbose)
  ...
  if (SolverType=="AztecOO")
    ...
    Solver->SetOutputStream(HYMLS::Tools::out());
    Solver->SetErrorStream(HYMLS::Tools::out());
    ...
    {
    HYMLS::Tools::Error("Invalid Solver Method: "+SolverType,__FILE__,__LINE__);
    }  
  return Solver;
  }
...
//    string name = AztecOO_uppercase((*pl_iter).first);
    ...
    if (!entry_used) 
      {
      HYMLS::Tools::Warning("your Aztec option '"+name+"' was not used!",__FILE__,__LINE__);
      }
    }
  }
...
void SolverFactory::DumpMLHierarchy(Teuchos::RCP<Epetra_Operator> P)
      ...
      for (int i=0;i<num_levels;i++)
        ...
        HYMLS::MatrixUtils::Dump(*Ai,fname.str());
        delete Ai;
        }
      }
...
void SolverFactory::AnalyzeSpectrum(Teuchos::ParameterList& plist, Teuchos::RCP<const Epetra_Operator> Prec)
  ...
  if (A==null) HYMLS::Tools::Error("SolverFactory::AnalyzeSpectrum: matrix pointer not set",__FILE__,__LINE__);
  ...
  HYMLS::Tools::Error("AnalyzeSpectrum: Could not create Anasazi Problem!",__FILE__,__LINE__);
  }
