...
! Set up Row- and Column Lump space for Preconditioner:
  IF (S%n /= x%Nschur) THEN
    PRINT '( /, A, I, A, I /)' , 'Number rows/columns in block diagonal: ', S%n, ' differs from number rows/columns in matrix: ', x%nschur
...
!   Insert the partition 'Part' at the end of the doubly linked list in 'x%mlp':
    ...
    IF (outlev >= 4) PRINT '(A, I3, A, 2(I7, 1X, A))', 'Step', npart + 1, ':', nupp, 'of the', NEqNotDon, 'unknowns eliminated approximately.'
...
!     at 'Part', and release workspace occupied by matrix S:
      IF ( density <= sparslim .AND. NEqNotDon >= 5 ) THEN
