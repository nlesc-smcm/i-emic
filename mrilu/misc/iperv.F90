...
SUBROUTINE iperv (invper, p, x, px)

USE m_dump

LOGICAL				, INTENT(IN)		:: invper
INTEGER, DIMENSION(1:)		, INTENT(IN)            :: p
INTEGER, TARGET, DIMENSION(1:)	, INTENT(IN)        	:: x
INTEGER, TARGET, DIMENSION(1:)	, INTENT(OUT)           :: px
...
!     ================
INTEGER 					:: i, ier
INTEGER, DIMENSION(:)	, POINTER       	:: ptr_x,ptr_px
LOGICAL						:: copbck
...
IF ( UBOUND(p,1) /= UBOUND(x,1) )   STOP 'Abnormal temination in iperv: incompatible upper bounds of p and x'
IF ( UBOUND(p,1) /= UBOUND(ptr_px,1) )  STOP 'Abnormal temination in iperv: incompatible upper bounds of p and ptr_px'
