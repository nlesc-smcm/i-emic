...
!      WRITE(99,*) "total nnz in A:", nnz
!      WRITE(99,*) "with ", count, " active connections out of ", nun*nun*np
      end   
...
!                  if ((jcoA(w+kk).lt.0).and.(abs(coA(w+kk)).gt.1.0D-14)) then
!                     write(*,*) i,j,k,i2,j2,k2,kk,ii,jj
!                  endif
               end if
            end do
         end do
...
!                  write(*,*) ii,jj,kk,XX
      ...
      enddo
      begA(ndim+1) = vn
*     write(99,999) oldsize,begA(ndim+1)-1
...
*****************************************************************************
      ...
      ALLOCATE(coB(ndim))
      ALLOCATE(begA(ndim+1))
      ALLOCATE(jcoA(ndim*(nun*np+1)))
      ALLOCATE(coA(ndim*(nun*np+1)))
...
*****************************************************************************
      ...
      ! deallocate matrixarrays
      USE m_mat
      IMPLICIT NONE
      INCLUDE 'usr.com'
      
      DEALLOCATE(Al, coB, begA, jcoA, coA)
      DEALLOCATE(active)
