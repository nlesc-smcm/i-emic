...
*     IMPORT/EXPORT
      integer type,i,j,k
      real    atom(n,m,l,np)
      real    cosdx2i(0:m),rdy2i,rdz2i,tand2(m),cosd2(m)
      real    h1,h2
...
C u_xx 
         ...
         do j = 1, m - 1
           do i= 1, n
            atom(i,j,:,2) = amh(y(j),ih)*cosdx2i(j)
            atom(i,j,:,8) = amh(y(j),ih)*cosdx2i(j)
...
C u_yy 
      ...
      CASE(4)
       ...
       CASE(5)
         tand2 = 1 - tan(y)*tan(y)
         DO j = 1, m-1
            atom(:,j,:,5) = bmh(y(j),ih)*tand2(j)+tan(y(j))*bmhy(y(j),ih)
         ENDDO
        CASE(6)
         tand2 = tan(y)
         cosd2 = cos(y)
         DO j = 1, m-1
         atom(:,j,:,2)=(bmhy(y(j),ih)-(amh(y(j),ih)+bmh(y(j),ih))*tand2(j))/(dx*cosd2(j))
         atom(:,j,:,8)=-(bmhy(y(j),ih)-(amh(y(j),ih)+bmh(y(j),ih))*tand2(j))/(dx*cosd2(j))
         ENDDO        
        CASE(7)
         IF (itopo.eq.3) then 
          DO i = 18,20
...
*     IMPORT/EXPORT
      integer type,i,j,k
      real    atom(n,m,l,np)
      real    cosdx2i(0:m),dy2i,rdz2i,cosd2(m),tand2(m)
      real    h1,h2
      real    amh,bmh,amhy,bmhy
*
      atom = 0.0
...
C vxx 
	 ...
	 do i=1,n
         do j = 1, m -1
            atom(i,j,:,2) = bmh(y(j),ih)*cosdx2i(j)
            atom(i,j,:,5) =-2*bmh(y(j),ih)*cosdx2i(j)
            atom(i,j,:,8) = bmh(y(j),ih)*cosdx2i(j)
         enddo
	 enddo
      CASE(3)
C vyy 
      ...
      CASE(4)
        ...
        CASE(5)
         DO j = 1, m-1
         atom(:,j,:,5)=bmh(y(j),ih)-amh(y(j),ih)*tan(y(j))*tan(y(j))+bmhy(y(j),ih)*tan(y(j))
         ENDDO
        CASE(6)
         tand2 = tan(y)
         cosd2 = cos(y)
         DO j = 1, m-1
         atom(:,j,:,2)=-((amh(y(j),ih)+bmh(y(j),ih))*tand2(j)-bmhy(y(j),ih))/(dx*cosd2(j))
         atom(:,j,:,8)= ((amh(y(j),ih)+bmh(y(j),ih))*tand2(j)-bmhy(y(j),ih))/(dx*cosd2(j))
         ENDDO  
      CASE(7)
         IF (itopo.eq.3) then
          DO i = 18,20
