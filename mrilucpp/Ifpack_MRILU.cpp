...
extern "C"
  ...
  void mrilucpp_set_params(const int* id, int* blocksize, int* cutmck, int* scarow, 
               ...
               double* nlsfctr, double* epsw, double* elmfctr, int* gusmod, double* gusfctr,double* redfctr,
               double* schtol, double* denslim, double* globfrac,double* locfrac, double* sparslim, int* ilutype,
               double* droptol, double* compfct, double* cpivtol, double* lutol_, int* singlu, int* outlev);
...
ApplyInverse(const Epetra_MultiVector& input,
  ...
  if (&input == &result) Error("aliased call to Ifpack_MRILU::ApplyInverse",__FILE__,__LINE__);
